//! Shared test fixtures: locate `bwa-mem2`, build a PhiX index, simulate
...
pub fn find_bwa_mem2() -> Option<String> {
...
/// Write the embedded PhiX sequence as a FASTA and build a bwa-mem2 index.
/// Returns the FASTA path (suitable as an index prefix for bwa-mem2-rs).
pub fn setup_phix_index(dir: &Path, bwa_mem2_bin: &str, phix_seq: &str) -> PathBuf {
    ...
    let status = Command::new(bwa_mem2_bin)
        .args(["index"])
        .arg(&ref_fa)
        .status()
