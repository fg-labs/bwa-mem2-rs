/* bwa-mem2-sys/shim/bwa_shim.h
 *
 * Public C API for the bwa-mem2 Rust FFI crate. Consumers of this header
 * should only see:
 ...
 * so bindgen does not have to parse the full bwa-mem2 C++ header graph
 ...
 * with bwa-mem2 defaults (mem_opt_init). `opts_free` releases it. Field-level
 ...
 * clear. Internally sets bwa-mem2's `bwa_rg_id[256]` global, which is
