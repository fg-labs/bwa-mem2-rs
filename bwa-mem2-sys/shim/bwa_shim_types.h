/* bwa-mem2-sys/shim/bwa_shim_types.h
 *
 * POD struct definitions copied from bwa-mem2's `bwamem.h` (mem_opt_t,
 * mem_pestat_t) so bindgen can emit full Rust struct layouts without
 * having to parse the C++ header graph rooted at bwamem.h.
 *
 * Keep the layouts byte-identical to bwa-mem2's. When refreshing the
 * vendored snapshot, diff against `vendor/bwa-mem2/src/bwamem.h` around
