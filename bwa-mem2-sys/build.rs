...
fn main() {
    ...
    let makefile_path = manifest.join("vendor/bwa-mem2/Makefile");
    ...
    assert!(
        ...
        "vendored bwa-mem2 Makefile must retain `-DMATE_SORT=0` default; shim pairing logic depends on it",
    );
    ...
    copy_dir(
        &manifest.join("vendor/bwa-mem2"),
        &build_dir.join("bwa-mem2"),
    );
    ...
    if patches_dir.is_dir() {
        ...
        for p in patches {
            ...
            let status = Command::new("patch")
                .current_dir(build_dir.join("bwa-mem2"))
    ...
    let vendor_src = build_dir.join("bwa-mem2/src");
    let ssl = build_dir.join("bwa-mem2/ext/safestringlib");
    let s2n = build_dir.join("bwa-mem2/ext/sse2neon");
