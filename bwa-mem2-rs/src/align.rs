...
fn to_c_pairs(pairs: &[ReadPair<'_>]) -> Vec<bwa_mem2_sys::BwaReadPair> {
    pairs
        .iter()
        .map(|p| bwa_mem2_sys::BwaReadPair {
...
pub struct Seeds {
    handle: *mut bwa_mem2_sys::BwaSeeds,
}

impl Drop for Seeds {
    fn drop(&mut self) {
        if !self.handle.is_null() {
            unsafe { bwa_mem2_sys::bwa_shim_seeds_free(self.handle) };
        }
    }
}
...
pub struct AlignmentBatch {
    handle: *mut bwa_mem2_sys::BwaBatch,
}

impl AlignmentBatch {
    pub fn len(&self) -> usize {
        unsafe { bwa_mem2_sys::bwa_shim_batch_n_records(self.handle) }
    }
    ...
    pub fn get(&self, i: usize) -> Record<'_> {
        let pair_idx = unsafe { bwa_mem2_sys::bwa_shim_batch_pair_idx(self.handle, i) };
        let ptr = unsafe { bwa_mem2_sys::bwa_shim_batch_record_ptr(self.handle, i) };
        let len = unsafe { bwa_mem2_sys::bwa_shim_batch_record_len(self.handle, i) };
...
impl Drop for AlignmentBatch {
    fn drop(&mut self) {
        if !self.handle.is_null() {
            unsafe { bwa_mem2_sys::bwa_shim_batch_free(self.handle) };
        }
    }
}
...
pub fn seed_batch(idx: &BwaIndex, opts: &MemOpts, pairs: &[ReadPair<'_>]) -> Result<Seeds> {
    ...
    let handle = unsafe {
        bwa_mem2_sys::bwa_shim_seed_batch(idx.raw(), opts.as_ptr(), c_pairs.as_ptr(), c_pairs.len())
    };
    if handle.is_null() {
...
pub fn extend_batch(
    ...
    let batch = unsafe {
        bwa_mem2_sys::bwa_shim_extend_batch(
            idx.raw(),
            opts.as_ptr(),
            s_handle,
            c_pairs.as_ptr(),
...
pub fn align_batch(
    ...
    let batch = unsafe {
        bwa_mem2_sys::bwa_shim_align_batch(
            idx.raw(),
            opts.as_ptr(),
            c_pairs.as_ptr(),
            c_pairs.len(),
...
pub fn estimate_pestat(
    ...
    let mut pestat_out = MemPeStat::zero()?;
    let rc = unsafe {
        bwa_mem2_sys::bwa_shim_estimate_pestat(
            idx.raw(),
            opts.as_ptr(),
            c_pairs.as_ptr(),
            c_pairs.len(),
